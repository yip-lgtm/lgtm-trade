#!/usr/bin/env python3
"""LSTM Backtest - Resample 15min to 1hr for more data points"""
import os
...
def load_15min_to_hourly(csv_file):
    """Load 15min CSV and resample to 1hr"""
    ...
    hourly = df.resample('1h').agg({'open':'first','high':'max','low':'min','close':'last','volume':'sum'}).dropna()
    return hourly
...
def train_and_predict_all(closes, seq_len=20):
    if len(closes) < seq_len + 50: return None
...
def run_backtest(sym, csv_file):
    ...
    df = load_15min_to_hourly(csv_file)
    if len(df) < 100:
        print(f"數據不足({len(df)}小時)")
        return []
    ...
    for i, (pred_price, actual_price) in enumerate(preds):
        ...
        trades.append({
            ...
            'hour': df.index[idx].strftime('%Y-%m-%d %H:00')
        })
...
print("=" * 60)
print("LSTM Backtest (15min -> 1hr, seq=20, thr=0.4, EMA+FVG)")
print("=" * 60)
all_trades = []
...
if all_trades:
    pd.DataFrame(all_trades).to_csv('/home/node/.openclaw/workspace/backtest_1hr.csv', index=False)
    ...
    print("已保存: /home/node/.openclaw/workspace/backtest_1hr.csv")
else:
    print("\n無交易")