#!/usr/bin/env python3
"""Run one symbol at a time to avoid timeout"""
...
sym = sys.argv[1] if len(sys.argv) > 1 else 'MES.F'
...
print(f'  Training ({x_tr.shape[0]} samples)...', flush=True)
...
for i in range(ts, len(df)):
    ...
    if not in_trade:
        if kz(bar.name) and hy!=0:
            in_trade=True; ep, eb, d = bar['close'], bar.name, hy
    else:
...
print(f'  {sym}: {len(trades)} trades in {time.time()-t0:.1f}s', flush=True)
pd.DataFrame(trades).to_csv(f'bt_{sym}.csv', index=False)
print(f'  Saved bt_{sym}.csv', flush=True)